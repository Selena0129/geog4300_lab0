...
svi<-read.csv("GEOG4300 LABS&CLASS stuff/geog4300_lab0/data/SVI_2022_US_county.csv")
tornado<-read.csv("GEOG4300 LABS&CLASS stuff/geog4300_lab0/data/stormevents/noaa_stormevent_tornado_2026_08.csv")
```
...
The first variable I chose was “COUNTY” from the SVI_2022_US_county.csv
file. It’s level of measurement is nominal because county names cannot
be arranged in a certain order, nor is there an agreed upon order they
could be categorized by. The county names are just a list and cannot be
used in calculations. The second variable I chose was “DAMAGE_PROPERTY”
from the noaa_stormevent_tornado_2026_08.csv file. It’s level of
measurement is ratio because these numeric values \[assuming we take the
“K” off the end of each number\] can make ratios, the difference between
values have meaning, and the data can be put in order from highest to
...
``` r
library(dplyr)
#I'm going to comment a few things during these tasks to keep track of what I'm doing just so you're aware. :)

#First, using a filter function to only keep F3,F4,F5 tornadoes from "TOR_F_SCALE."
TOR_F_SCALE<-tornado %>%
filter(TOR_F_SCALE %in% c("F3", "F4", "F5")) %>%
#Second, using the group function to group the data by the county and stata_fips, codes and names.
group_by(STATE_FIPS, STATE, cty_fips, CZ_NAME) %>%
#Third, using the summarise function to county the number of tornadoes in the groups. 
summarise(TOR_F_SCALE = n())
```
...
``` r
#I figured out that I couldn't run each line of code separately when using "%>%", or else I just get an error. Noting for next time.
```
...
\#The variable I want is “E_TOTPOP” -\> this was on page 7 of the SVI
pdf document. Filter the data below.

**Task 4:** *Filter the SVI data to include counties with a population
of 25,000 or more.*

``` r
#Using the filter function to include counties with a total population of 25000 or more. 
...
``` r
#Choosing F_NOVEH, so for the filtered counties of only 25000+ people, keep on the counties that are flagged as being in the top 10% for households with vehicle access. The rows being marked with a "1" counts. May seem counter intuitive to pair vehicle and tornado, but people could drive to shelters when they know a tornado is coming. Using the filter function again here.
...
I chose the variable “F_NOVEH” which flags the counties – out of the
ones with 25000 people or more – within the 90th percentile or higher
for the percentage of households with no access to a vehicle. I chose it
because I wanted to pick something different from perhaps the “obvious”
ones like mobile homes or disability vulnerabilities (and wanted to
choose one that your mind maybe wouldn’t immediately go too). “Vehicle”
and “tornado” don’t sounds good in the same sentence, so for this case,
the people without access to a vehicle could mean one less option to
reach a proper tornado shelter/the ability to evacuate a dangerous area.
Counties that were flagged with a “1” were counted as vulnerable, so I
don’t think I needed to create an additional cutoff.
...
``` r
#Changed the given "svi" variable to my chosen variable "F_NOVEH." Then using the inner_join function below. 
F_NOVEH <-F_NOVEH %>%
...
``` r
#Using the inner join function to join the the filtered tornado data and the county data (F_NOVEH). 
joined_data<-inner_join(TOR_F_SCALE, F_NOVEH, by = "cty_fips")
```
...
``` r
#Used the ?inner_join function to answer this question. 
```

The “inner_join” function two data sets based on matching columns, yet
it only keeps the columns that have in common, leaves out the rest. This
is not super applicable in most cases as it would be very easy to lose
data that could be of value. In comparison with “full_join,” they
combine both datasets without there having to be a match from the other
dataset. Full joins would also contain “NA” values where a dataset did
...
#Either I missed something above in my code or something else... I used the arrange function to see the three vulnerable counties that have the higher number of severe tornadoes, they were not in order from just the joined data for some reason.
```

1.  Caddo Parish County, Louisiana – 10. In the northwest of the state,
    bordering Texas and Arkansas.
2.  Mississippi County, Arkansas – 9. A relatively flat area, downslope
    of the Boston Mountains and Ouachita Mountains.
3.  Woodruff County, Arkansas – 9. Lies on the Ozark Plateau.
...
I had some inputs from classmates outside of class. I also used AI
assistance (ChatGPT) to tell me what exact R is doing when using certain
functions when I couldn’t remember what the core meaning was for a
function. Using the AI assistance supported my learning of this new
coding language greatly as I am now able to understand what R is doing
behind the scene when I input certain functions. Getting input from
other outside the class was helpful in the process of how to set up/work
with GitHub Desktop and the whole pushing/editing process.

**Lab reflection:** How do you feel about the work you did on this lab?
Was it easy, moderate, or hard? What are the biggest things you learned
by completing it?

On a scale of 1-10, I’d say a 6, and the work on this lab was moderate.
It was not in the moderate category for the tasks themselves per say,
more like I felt it took me extra time to get familiar with the basics
of R (I am only used to python) and fully understand what the functions
were doing. The biggest thing I learned during this lab was about myself
actually. I have to slow down and really take the time to learn from
mistakes while completing the assignment - trying to rush through
something like this is NOT the way to go.
